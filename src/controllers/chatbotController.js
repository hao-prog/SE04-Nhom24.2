...
let setUpUserFacebookProfile = (req, res) => {
    ...
    homepageService.setUpMessengerPlatform(PAGE_ACCESS_TOKEN);
...
function handlePostback(sender_psid, received_postback) {
    ...
    switch (payload) {
        ...
        case 'GET_STARTED':
            ...
            break;
        case 'dungvay':
            let randNum1 = Math.floor(Math.random() * Object.keys(askNameQs).length) + 1;
            let txt1 = askNameQs[randNum1];
            response = { "text": txt1 }
            break;
        case 'khongphai':
            response = { "text": "Gut pye" }
            break;
        case 'canhan': {
            ansArr[0] = "Cá nhân";
            let randNum2 = Math.floor(Math.random() * Object.keys(askPhoneNumQs).length) + 1;
            let txt2 = askPhoneNumQs[randNum2];
            response = { "text": txt2 };
            break;

        } case 'tochuc':
            ansArr[0] = "Tổ chức";
            let randNum3 = (Math.floor(Math.random() * Object.keys(askOrgQs).length) + 1);
            let txt3 = askOrgQs[randNum3];
            response = { "text": txt3 };
            break;
        case 'donvikhac':
            ansArr[0] = "Tổ chức";
            let randNum4 = Math.floor(Math.random() * Object.keys(askOtherOrgQs).length) + 1;
            let txt4 = askOtherOrgQs[randNum4];
            response = { "text": txt4 };
            break;
        default:
            console.log("No case payload");
    }
